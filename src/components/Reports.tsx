...
import { Printer, FileText, Calendar, Loader2, Sparkles, Building2 } from 'lucide-react';
...
export default function Reports({ role }: ReportsProps) {
  ...
  const [reportType, setReportType] = useState<'quadrangular' | 'financial' | 'cells' | 'meetings'>(
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between print:hidden">
        <div>
          <h1 className="text-3xl font-bold text-zinc-900">Relatórios Oficiais</h1>
          <p className="text-zinc-500">Gere, divida cultos e imprima relatórios no formato da igreja</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex rounded-xl border border-zinc-200 bg-white p-1 shadow-xs">
            {role !== 'cell' && (
              <button
                onClick={() => {
                  setReportType('quadrangular');
                  setSearchParams({ type: 'quadrangular' });
                }}
                className={cn(
                  "flex items-center gap-1.5 rounded-lg px-3.5 py-1.5 text-xs font-bold transition-all",
                  reportType === 'quadrangular' ? "bg-blue-600 text-white shadow-xs" : "text-zinc-600 hover:text-zinc-900"
                )}
              >
                <Sparkles size={14} />
                Modelo Oficial Quadrangular (REFC)
              </button>
            )}
            {role !== 'cell' && (
              <button
            ...
            <>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="rounded-lg border border-zinc-200 bg-white py-1.5 pl-9 pr-3 text-xs font-semibold outline-none focus:border-zinc-900"
                />
              </div>

              <button
                onClick={handlePrint}
                className="flex items-center gap-1.5 rounded-lg bg-zinc-900 px-3.5 py-1.5 text-xs font-semibold text-white transition-all hover:bg-zinc-800 active:scale-95"
              >
                <Printer size={16} />
                Imprimir A4
              </button>
      ...
      {/* SELECIONOU MODELO OFICIAL QUADRANGULAR (REFC & ENTRADAS) */}
      {reportType === 'quadrangular' ? (
        <QuadrangularReport role={role} />
      ) : (
        /* Report Preview / Print Area (Outros formatos) */
        <div 
          ref={printRef}
        ...
        >
          ...
          <div className="mb-12 flex items-center justify-between border-b-2 border-zinc-900 pb-8">
            <div className="flex items-center gap-6">
              ...
              <div id="logo-fallback" className={cn("flex h-24 w-24 items-center justify-center rounded-xl bg-zinc-100 text-zinc-400", (settings?.logoUrl && !logoError) ? "hidden" : "")}>
                <FileText size={40} />
              </div>
              <div>
      ...
      <style>{`
        @media print {
          ...
          .print\\:m-0, .print\\:m-0 *, #print-refc-sheet, #print-refc-sheet *, #print-entradas-sheet, #print-entradas-sheet * {
            visibility: visible;
          }
          .print\\:m-0 {
            position: absolute;
